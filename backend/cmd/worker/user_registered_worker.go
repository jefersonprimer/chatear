...
func main() {
	...
	emailService := notificationApp.NewEmailService(smtpSender, oneTimeTokenService, cfg.AppURL, cfg.MagicLinkExpiry, emailLimiter)

	consumer := notificationWorker.NewUserRegisteredConsumer(emailService, cfg.AppURL)

	_, err = infra.NatsConn.Subscribe(events.UserRegisteredSubject, func(msg *nats.Msg) {
	...
	log.Println("User registered worker started. Waiting for events...")
	...
	log.Println("User registered worker stopped.")
}

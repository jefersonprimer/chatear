...
func main() {
	...
	consumer := notificationWorker.NewAccountDeletionConsumer(emailService)

	_, err = infra.NatsConn.Subscribe(events.AccountDeletionRequestedSubject, func(msg *nats.Msg) {
	...
	log.Println("Account deletion worker started. Waiting for events...")
	...
	log.Println("Account deletion worker stopped.")
}
